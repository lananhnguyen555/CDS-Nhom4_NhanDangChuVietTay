...
def main(config, device, logger, vdl_writer, seed):
    ...
    if hasattr(post_process_class, "character"):
        ...
        if config["Architecture"]["algorithm"] in [
            ...
            out_channels_list["CTCLabelDecode"] = char_num
            # update SARLoss params
            loss_config_list = config["Loss"].get("loss_config_list", [])
            if len(loss_config_list) > 1 and list(loss_config_list[1].keys())[0] == "SARLoss":
                ...
                out_channels_list["SARLabelDecode"] = char_num + 2
            elif len(loss_config_list) > 1 and list(loss_config_list[1].keys())[0] == "NRTRLoss":
                out_channels_list["NRTRLabelDecode"] = char_num + 3
