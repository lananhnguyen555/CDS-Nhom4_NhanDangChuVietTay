...
# Lazy import albumentations only when needed
def _import_albumentations():
    global A, DualTransform, fgeometric, version, ALBUMENTATIONS_AVAILABLE, ALBU_VERSION, IS_ALBU_NEW_VERSION
...
class ImgaugLikeResize:
    def __init__(self, scale_range=(0.5, 3.0), interpolation=1, p=1.0):
        _import_albumentations()
        if ALBUMENTATIONS_AVAILABLE:
            # Use DualTransform from albumentations if available
            self.base_class = DualTransform
        else:
            self.base_class = DualTransformStub
        self.p = p
...
class IaaAugment:
    def __init__(self, augmenter_args=None, **kwargs):
        _import_albumentations()  # Try to import when needed
        if not ALBUMENTATIONS_AVAILABLE:
            self.augmenter = None
            return
    ...
    def __call__(self, data):
        if not ALBUMENTATIONS_AVAILABLE or self.augmenter is None:
            return data  # Return data unchanged if augmentation not available
        image = data["image"]
